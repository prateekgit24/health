    ...
    <article
      className={`app-surface rounded-2xl p-5 ${
        food.verified && isVeryCommonFood(food)
          ? "border-emerald-300/70 bg-[linear-gradient(150deg,rgba(16,185,129,0.28),rgba(8,30,21,0.72))]"
          : ""
      }`}
    >
      <Link href={`/nutrition/${foodSlug(food.name)}`} className="block">
        <div className="flex items-start justify-between gap-4">
          <h2 className="text-2xl font-semibold text-emerald-100">{food.name}</h2>
          <p className="text-sm text-slate-300">{food.serving}</p>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="rounded-full bg-emerald-500/25 px-2.5 py-1 text-xs font-semibold text-emerald-100">{food.diet_token}</span>
          {food.is_common ? (
          ...
          {food.verified ? (
            <span className="rounded-full bg-lime-500/25 px-2.5 py-1 text-xs font-semibold text-lime-100">
              USDA verified
            </span>
          ) : (
            <span className="rounded-full bg-rose-500/25 px-2.5 py-1 text-xs font-semibold text-rose-100">
      ...
      <button
        ...
        className={`mt-4 rounded-full px-4 py-1.5 text-xs font-semibold ${
          isSelected
            ...
            : "app-pill text-emerald-50 hover:border-emerald-200/45 hover:text-white"
    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-6">
      ...
      <section className="app-surface mt-6 grid gap-3 rounded-2xl p-4 md:grid-cols-2 lg:grid-cols-3">
        <label className="text-sm font-semibold text-emerald-100">
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-emerald-200/30 bg-[#07241a]/90 px-3 py-2 text-sm text-emerald-50 placeholder:text-slate-400"
          />
        </label>
        <label className="text-sm font-semibold text-emerald-100">
          Category
          <select
            ...
            className="mt-1 w-full rounded-lg border border-emerald-200/30 bg-[#07241a]/90 px-3 py-2 text-sm text-emerald-50"
          >
            {categories.map((item) => (
      ...
      <section className="app-surface mt-6 rounded-2xl p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-xl font-semibold text-emerald-100">Compare Foods (up to 5)</h2>
          <button
            type="button"
            onClick={() => setCompareNames([])}
            className="app-pill rounded-full px-4 py-1.5 text-xs font-semibold text-emerald-50 hover:border-emerald-200/45 hover:text-white"
          >
            Clear compare
          </button>
        </div>
        {compareFoods.length === 0 ? (
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="min-w-full text-sm text-slate-100">
              <thead>
                <tr>
                  <th className="border-b border-emerald-200/20 px-2 py-2 text-left">Metric</th>
                  {compareFoods.map((food) => (
                    <th key={food.name} className="border-b border-emerald-200/20 px-2 py-2 text-left text-emerald-100">
                      {food.name}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                <tr>
                  <td className="border-b border-emerald-200/15 px-2 py-2">Calories (kcal)</td>
                  {compareFoods.map((food) => <td key={`cal-${food.name}`} className="border-b border-emerald-200/15 px-2 py-2">{food.calories}</td>)}
                </tr>
                <tr>
                  <td className="border-b border-emerald-200/15 px-2 py-2">Protein (g)</td>
                  {compareFoods.map((food) => <td key={`pro-${food.name}`} className="border-b border-emerald-200/15 px-2 py-2">{food.protein}</td>)}
                </tr>
                <tr>
                  <td className="border-b border-emerald-200/15 px-2 py-2">Carbs (g)</td>
                  {compareFoods.map((food) => <td key={`carb-${food.name}`} className="border-b border-emerald-200/15 px-2 py-2">{food.carbs}</td>)}
                </tr>
                <tr>
                  <td className="border-b border-emerald-200/15 px-2 py-2">Fat (g)</td>
                  {compareFoods.map((food) => <td key={`fat-${food.name}`} className="border-b border-emerald-200/15 px-2 py-2">{food.fat}</td>)}
                </tr>
                <tr>
                  <td className="border-b border-emerald-200/15 px-2 py-2">Fiber (g)</td>
                  {compareFoods.map((food) => <td key={`fib-${food.name}`} className="border-b border-emerald-200/15 px-2 py-2">{food.fiber}</td>)}
                </tr>
                <tr>
                  <td className="border-b border-emerald-200/15 px-2 py-2">Sugar (g)</td>
                  {compareFoods.map((food) => <td key={`sug-${food.name}`} className="border-b border-emerald-200/15 px-2 py-2">{food.sugar}</td>)}
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {filteredFoods.length === 0 ? (
        <p className="mt-6 rounded-xl border border-emerald-300/35 bg-emerald-500/15 p-4 text-sm text-emerald-100">
